...
class DataModule(pl.LightningDataModule):
    def __init__(self, params):
        ...
        self.params.train_dataset_path = os.path.join(
            self.params.dataset_path, self.params.train_dataset_name
        )
        self.params.test_dataset_path = os.path.join(
            self.params.dataset_path, self.params.dataset_name
        )
            ...
            self.train_transform = transforms.Compose(
                [
                    transforms.RandomCrop(
                        self.params.train_patch_size, pad_if_needed=True
                    ),
        ...
        if self.params.test_resize:
            if self.params.test_resize_mode == "center_crop":
                ...
                val_transform_list = [
                    transforms.RandomCrop(
                        self.params.test_patch_size, pad_if_needed=True
                    )
                ]
            else:
                val_transform_list = [transforms.Resize(self.params.test_patch_size)]
        ...
        self.dataset_train = datasets.ImageFolder(
            root=self.params.train_dataset_path, transform=self.train_transform
        )
        self.dataset_val = datasets.ImageFolder(
            root=self.params.test_dataset_path, transform=self.val_transform
        )
    ...
    def add_data_specific_args(parent_parser):
        ...
        parser.add_argument("--train_dataset_name", type=str, default="DRUNET")
        ...
        parser.add_argument(
            "--train_shuffle", dest="train_shuffle", action="store_true"
        )
        parser.add_argument(
            "--no-train_shuffle", dest="train_shuffle", action="store_false"
        )
        parser.set_defaults(train_shuffle=True)
        parser.add_argument(
            "--no_test_resize", dest="test_resize", action="store_false"
        )
