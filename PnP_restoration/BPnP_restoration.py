...
from utils.utils_restoration import (
    burg_bregman_divergence,
    burg_bregman_divergence_np,
    psnr,
    array2tensor,
    tensor2array,
)
...
class PnP_restoration:
    ...
    def initialize_prox(self, img, degradation):
        ...
        """
        if (
            self.hparams.degradation_mode == "deblurring"
            or self.hparams.degradation_mode == "SR"
        ):
            k = degradation
            self.k_tensor = torch.tensor(k).to(self.device)
            self.FB, self.FBC, self.F2B, self.FBFy = utils_sr.pre_calculate_prox(
                img, self.k_tensor, self.hparams.sf
            )
    ...
    def data_fidelity_grad(self, x, y):
        ...
        """
        return utils_sr.grad_solution_KL(
                x, y, self.k_tensor, self.hparams.sf, self.hparams.alpha_poisson
            )
    ...
    def data_fidelity_grad_step(self, x, y, stepsize):
        ...
        """
        grad = utils_sr.grad_solution_KL(
                x, y, self.k_tensor, self.hparams.sf, self.hparams.alpha_poisson
            )
        if self.hparams.bregman_potential == "L2":
    ...
    def calculate_regul(self, y, x, g=None, prox_pnp=True):
        ...
        """
        if g is None:
            _, _, g = self.denoise(x, self.sigma_denoiser)
        if not prox_pnp:
            return self.alpha * g
        else:
            if self.hparams.bregman_potential == "L2":
    ...
    def calculate_lyapunov_DRS(self, y, z, x, g, img):
        ...
        Psi = (
            ...
            + (1 / self.lamb)
            * (
                torch.sum(torch.mul(y - x, y - z))
                + (1 / 2) * torch.norm(y - z, p=2) ** 2
            )
        )
        return Psi.item()

    def restore(
        self, img, init_im, clean_img, degradation, extract_results=False, sf=1
    ):
        """
        ...
        if self.hparams.use_linear_init:
            x0 = self.At(x0)
        if (
            self.hparams.bregman_potential == "burg"
        ):  # Burg entropy is only define with strictly positive values
        ...
        if extract_results:  # extract numpy images and PSNR values
            ...
            if (
                self.hparams.lim_init == 0
            ):  # if no initialization phase, save the initialization
        ...
        if (
            self.hparams.lim_init == 0
        ):  # if no initialization phase, initialize the parameters with the algorithm parameter values.
            init = (
                False
            )  # init is True if we are in the initialization phase at the current iteration
            prev_init = (
                False
            )  # prev_init is True if we were in the initialization phase at the previous iteration
        ...
        while i < self.hparams.maxitr:  # for each iteration
            ...
            if (
                not init and prev_init
            ):  # if initialization phase is over, set the parameters to the algorithm parameter values.
                init = False
                self.tau = self.hparams.stepsize
                self.lamb = self.hparams.lamb
                self.sigma_denoiser = self.hparams.sigma_denoiser
            ...
            if self.hparams.PnP_algo == "GD":
                ...
                _, Dg, g = self.denoise(x_old, self.sigma_denoiser)
                # Gradient step
                if self.hparams.bregman_potential == "L2":
                    ...
                    x = (
                        (den) * (0 <= den) * (den <= self.hparams.C_max)
                        + self.hparams.C_max * (den < 0)
                        + self.hparams.C_max * (den > self.hparams.C_max)
                    )
                else:
                    raise ValueError("Bregman potential not implemented")
                y = z = x
                ...
                # Denoising step
                Dz, Dg, g = self.denoise(z, self.sigma_denoiser, weight=self.alpha)
                x = Dz
                y = x
                # Hard constraint
                if self.hparams.use_hard_constraint:
    ...
    def update_curves(
        self, x_list, psnr_tab, Dg_list, g_list, F_list, Psi_list, f_list, Df_list
    ):
        ...
        self.conv.append(
            np.array(
                [
                    (np.linalg.norm(x_list[k + 1] - x_list[k]) ** 2)
                    for k in range(len(x_list) - 1)
                ]
            )
            / np.sum(np.abs(x_list[0]) ** 2)
        )
        self.lip_algo.append(
            np.sqrt(
                np.array(
                    [
                        np.sum(np.abs(x_list[k + 1] - x_list[k]) ** 2)
                        for k in range(1, len(x_list) - 1)
                    ]
                )
                / np.array(
                    [
                        np.sum(np.abs(x_list[k] - x_list[k - 1]) ** 2)
                        for k in range(1, len(x_list[:-1]))
                    ]
                )
            )
        ...
        self.conv_Dh.append(
            np.array(
                [
                    (burg_bregman_divergence_np(x_list[k + 1], x_list[k]))
                    for k in range(len(x_list) - 1)
                ]
            )
            / np.sum(np.abs(x_list[0]) ** 2)
    ...
    def save_curves(self, save_path):
        ...
        for i, algo in enumerate(self.hparams.PnP_algos):
            Ds_norm = [
                np.linalg.norm(np.array(self.Dg[i][j])) for j in range(len(self.Dg[i]))
            ]
            plt.plot(
                Ds_norm,
                label=label_dict[algo],
        ...
        for i, algo in enumerate(self.hparams.PnP_algos):
            Ds_norm = [
                np.linalg.norm(np.array(self.Df[i][j])) for j in range(len(self.Df[i]))
            ]
            plt.plot(
                Ds_norm,
                label=label_dict[algo],
        ...
        conv_rate = [
            self.conv[i][0] * np.array([(1 / k) for k in range(1, len(self.conv[i]))])
        ...
        self.conv2 = [
            [np.min(self.conv[i][:k]) for k in range(1, len(self.conv[i]))]
            for i in range(len(self.conv))
        ]
        plt.figure(6)
        fig, ax = plt.subplots()
        ...
        self.conv_sum = [
            [np.sum(self.conv[i][:k]) for k in range(1, len(self.conv[i]))]
            for i in range(len(self.conv))
        ]
    ...
    def add_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--use_GSDRUNET_ckpt", dest="use_GSDRUNET_ckpt", action="store_true"
        )
        parser.set_defaults(use_GSDRUNET_ckpt=False)
        parser.add_argument("--PnP_algos", type=str, nargs="+", required=True)
        parser.add_argument("--alpha_poisson", type=float, required=True)
        ...
        parser.add_argument(
            "--use_backtracking", dest="use_backtracking", action="store_true"
        )
        ...
        parser.add_argument(
            "--inpainting_init", dest="inpainting_init", action="store_true"
        )
        ...
        parser.add_argument(
            "--extract_curves", dest="extract_curves", action="store_true"
        )
        parser.set_defaults(extract_curves=False)
        parser.add_argument(
            "--extract_images", dest="extract_images", action="store_true"
        )
        parser.set_defaults(extract_images=False)
        parser.add_argument(
            "--print_each_step", dest="print_each_step", action="store_true"
        )
        ...
        parser.add_argument(
            "--use_hard_constraint", dest="use_hard_constraint", action="store_true"
        )
        parser.set_defaults(use_hard_constraint=False)
        parser.add_argument("--use_wandb", dest="use_wandb", action="store_true")
        parser.set_defaults(use_wandb=False)
        parser.add_argument(
            "--use_Wiener_init", dest="use_Wiener_init", action="store_true"
        )
        ...
        parser.add_argument("--lamb_pre_init", type=float, default=1.0)
        parser.add_argument("--sigma_denoiser_pre_init", type=float, default=50.0)
        parser.add_argument("--stepsize_pre_init", type=float, default=1.0)
        parser.add_argument("--alpha_pre_init", type=float, default=1.0)
        return parser
