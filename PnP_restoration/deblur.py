...
from utils.utils_restoration import rescale, array2tensor, tensor2array, get_gaussian_noise_parameters, get_poisson_noise_parameters, create_out_dir, single2uint,crop_center, matlab_style_gauss2D, imread_uint, imsave
        ...
        for PnP_algo in hparams.PnP_algos :
            ...
            if hparams.noise_model == 'gaussian':
                if PnP_algo == 'PGD_RED' or  PnP_algo == 'GD':
                    ...
                    PnP_module.hparams.maxitr = 80
                else:
                    PnP_module.use_GSDRUNET_ckpt = False
                    PnP_module.hparams.maxitr = 500
                PnP_module.initialize_denoiser()
            # if 'RED' in PnP_algo:
            #     PnP_module.hparams.use_backtracking = True 

            if hparams.default_params:
                if hparams.noise_model == 'gaussian':
                    PnP_module.hparams.lamb, PnP_module.hparams.sigma_denoiser, PnP_module.hparams.alpha, PnP_module.hparams.alpha_pd = get_gaussian_noise_parameters(hparams.PnP_algo, hparams.noise_level_img, k_index=k_index, degradation_mode='deblur')
                elif hparams.noise_model == 'poisson':
                    PnP_module.hparams.lamb_pre_init, PnP_module.hparams.sigma_denoiser_pre_init, PnP_module.hparams.stepsize_pre_init, PnP_module.hparams.lim_init, PnP_module.hparams.sigma_denoiser, PnP_module.hparams.lamb, PnP_module.hparams.stepsize = get_poisson_noise_parameters(hparams.PnP_algo, hparams.bregman_potential, hparams.alpha_poisson, hparams.use_GSDRUNET_ckpt)
                else:
            ...
            for i in range(min(hparams.n_images,len(input_paths))): # For each image
                print('Deblurring of image {}, kernel index {}, with PnP-{} algorithm'.format(i,k_index,PnP_algo))
                ...
                if hparams.noise_model == 'gaussian' :
                    ...
                    blur_im = np.random.poisson(np.maximum(hparams.alpha_poisson*blur_im,0)) / hparams.alpha_poisson
                else :
                    raise ValueError('Noise model not implemented')      
                blur_im =  np.float32(blur_im)

                if hparams.use_Wiener_init and hparams.noise_model == 'poisson' : # Wiener filter initialzation for Poisson noise
                    init_im = Wiener_filter(array2tensor(blur_im), torch.tensor(k), 1/hparams.alpha_poisson, 1)
                ...
                # PnP restoration
                if hparams.extract_images or hparams.extract_curves or hparams.print_each_step:
                    deblur_im, init_im, output_psnr, n_it, x_list, z_list, y_list, Dg_list, psnr_tab, g_list, F_list, Psi_list, f_list, Df_list = PnP_module.restore(blur_im.copy(),init_im.copy(),input_im.copy(),k, extract_results=True)
                else :
