...
def inpaint():
    ...
    for i in range(0,1):
        ...
        if hparams.extract_images or hparams.extract_curves or hparams.print_each_step:
            inpainted_im, output_psnr, output_psnrY, x_list, z_list, y_list, Dg_list, psnr_tab, Dx_list, g_list, F_list, Psi_list = PnP_module.restore(mask_im.copy(),init_im.copy(),input_im.copy(),mask, extract_results=True)
        else:
            inpainted_im, output_psnr, output_psnrY = PnP_module.restore(mask_im, init_im, input_im, mask)
        ...
        if hparams.extract_curves:
            # Create curves
            PnP_module.update_curves(x_list, Dx_list, psnr_tab, Dg_list, g_list, F_list, Psi_list)
