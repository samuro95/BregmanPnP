...
def get_poisson_noise_parameters(PnP_algo, bregman_potential, alpha_poisson, use_GSDRUNET_ckpt):
    if PnP_algo == 'GD':
        if hparams.bregman_potential == 'burg' :
            sigma_denoiser_pre_init = 50
            stepsize_pre_init = 1.
            sigma_denoiser = 500
            lim_init = 100
            stepsize = 0.05
            if PnP_module.hparams.alpha_poisson == 5 :
                lamb_pre_init = 0.5
                lamb = 0.1
            if PnP_module.hparams.alpha_poisson == 10 :
                lamb_pre_init = 1.
                lamb = 0.1
            if PnP_module.hparams.alpha_poisson == 20 :
                lamb_pre_init = 1.5
                lamb = 0.5
            if PnP_module.hparams.alpha_poisson == 40 :
                lamb_pre_init = 2.
                lamb = 0.5
            if PnP_module.hparams.alpha_poisson == 60 :
                lamb_pre_init = 2.5
                lamb = 0.5
    if PnP_algo == 'PGD':
        ...
        elif hparams.bregman_potential == 'L2':
            if alpha_poisson == 5 :
                sigma_denoiser = 0.05
                lamb = 0.02
            if alpha_poisson == 10 :
    ...
    return lamb_pre_init, sigma_denoiser_pre_init, stepsize_pre_init, lim_init, sigma_denoiser, lamb, stepsize

def get_gaussian_noise_parameters(PnP_algo, noise_level_img, k_index=0, degradation_mode='deblur'):
    '''
    ...
    if PnP_algo == 'PGD_RED': 
        ...
        alpha = 0.5
        if noise_level_img/255. < 0.02:
            lamb = 5
            sigma_k_denoiser = 2
        elif noise_level_img/255. < 0.04:
            lamb = 1.5
            sigma_k_denoiser = 1
        else:
            lamb = 1.
        ...
        if noise_level_img/255. < 0.02:
            ...
            sigma_k_denoiser = 0.5
        M = alpha/(alpha+1)
        roots = np.roots(np.array([lamb,0,-(2+M),2*M]))
        roots = roots[(roots>0)*(roots<(1/lamb))]
        if len(roots) == 1 :
            alpha_pd_max = (1/lamb)
        elif len(roots) == 2 :
            alpha_pd_max = np.amax(roots)
        else :
            raise ValueError
        # to comment for other bound 
        alpha_pd = alpha_pd_max-eps
        #print(M, hparams.alpha_pd, alpha_pd_max)
    else :
        alpha = 1.
        lamb = 1.
        sigma_k_denoiser = 1.
    ...
    return lamb, sigma_denoiser / 255. , alpha, alpha_pd
